...
const buildFullGrid = (data: CoverageInterval[]) => {
  ...
  const grid: { label: string; delta: number; hour: number }[] = [];
  ...
  for (let d = 0; d < 7; d++) {
    ...
    for (let h = STORE_OPEN; h < STORE_CLOSE; h++) {
      ...
      grid.push({ label: `${dayName} ${ampm}`, delta: point?.delta ?? 0, hour: h });
    }
  }
  return grid;
};
...
export const CoverageDeltaChart = ({ data }: Props) => {
  ...
  return (
    <div className="rounded-2xl border border-border bg-card p-5">
      <h3 className="text-base font-semibold text-foreground">Coverage Gap</h3>
      <p className="mb-4 text-xs text-muted">
        Staffing surplus or deficit per hour. <span className="text-success">Green</span> = overstaffed, <span className="text-danger">red</span> = understaffed.
      </p>
      <ResponsiveContainer width="100%" height={260}>
        <BarChart data={chartData} margin={{ left: 0, right: 8, top: 4, bottom: 0 }}>
          ...
          <Tooltip
            ...
            formatter={(value) => [`${Number(value) > 0 ? "+" : ""}${value} workers`, "Delta"]}
          />
          <ReferenceLine y={0} stroke="#9ca3af" strokeWidth={1} />
          <Bar dataKey="delta" radius={[2, 2, 0, 0]} maxBarSize={6}>
            {chartData.map((entry, idx) => (
              <Cell key={idx} fill={entry.delta >= 0 ? "#059669" : "#dc2626"} />
            ))}
          </Bar>
        </BarChart>
      </ResponsiveContainer>
